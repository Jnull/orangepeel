...
localforage.getItem('master_delete_rules').then(function (result, err) {
    ...
    var observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            ...
            if (xpath_array_rules && ~xpath_array_rules.indexOf(mutated_element_xpath) &&
                Array.from(mutation.target.classList).indexOf('hide_personal_element') === -1 && (!mutation.target.dataset.bypassObserver)) {
